...
# Хранилище для ожидающих ответов админа
waiting_for_design: Dict[int, int] = {}  # {admin_id: user_id}
...
async def send_question(message: Message, text: str, buttons=None):
    if buttons:
...
async def new_project(message: Message, state: FSMContext):
    await state.clear()
    await send_question(message, "📋 *Вопрос 1/20: Какую комнату вы хотите оформить?*")
    await message.answer(
...
async def q1(call: CallbackQuery, state: FSMContext):
    ...
    await send_question(call.message, "📏 *Вопрос 2/20: Какая площадь комнаты?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["До 12 м²", "12-20 м²", "20+ м²", "Не знаю"], "area", 2)
    )
...
async def q2(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "🪟 *Вопрос 3/20: Сколько окон в комнате?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Нет окон", "1 окно", "2 окна", "Больше 2"], "windows", 2)
    )
...
async def q3(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "🎨 *Вопрос 4/20: Какой стиль интерьера вам ближе?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Современный", "Минимализм", "Лофт", "Скандинавский", "Классика", "Прованс", "Бохо", "Эко-стиль", "Свой вариант"], "style", 2)
    )
    await state.set_state(Form.q4_style)
    await call.answer()

@dp.callback_query(Form.q4_style, F.data == "style_Свой вариант")
async def custom_style(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await send_question(call.message, "✏️ Напишите ваш вариант стиля текстом:")
    await state.set_state(Form.q4_style)
    await call.answer()

@dp.callback_query(Form.q4_style, F.data.startswith("style_"))
async def q4_chosen(call: CallbackQuery, state: FSMContext):
    await state.update_data(style=call.data.split("_")[1])
    await call.message.delete()
    await send_question(call.message, "🧘 *Вопрос 5/20: Какое настроение вы хотите создать?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Уютное", "Строгое", "Романтичное", "Игривое", "Яркое", "Спокойное", "Минималистичное", "Другое"], "mood", 2)
    )
    await state.set_state(Form.q5_mood)
    await call.answer()

@dp.message(Form.q4_style)
async def custom_style_text(msg: Message, state: FSMContext):
    await state.update_data(style=msg.text)
    await send_question(msg, "🧘 *Вопрос 5/20: Какое настроение вы хотите создать?*")
    await msg.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Уютное", "Строгое", "Романтичное", "Игривое", "Яркое", "Спокойное", "Минималистичное", "Другое"], "mood", 2)
    )
...
@dp.callback_query(Form.q5_mood, F.data == "mood_Другое")
async def custom_mood(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await send_question(call.message, "✏️ Опишите желаемое настроение своими словами:")
    await state.set_state(Form.q5_mood)
    await call.answer()
...
async def q5(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "💰 *Вопрос 6/20: Какой бюджет на проект?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Эконом", "Средний бюджет", "Премиум", "Без разницы"], "budget", 2)
    )
    await state.set_state(Form.q6_budget)
    await call.answer()

@dp.message(Form.q5_mood)
async def custom_mood_text(msg: Message, state: FSMContext):
    await state.update_data(mood=msg.text)
    await send_question(msg, "💰 *Вопрос 6/20: Какой бюджет на проект?*")
    await msg.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Эконом", "Средний бюджет", "Премиум", "Без разницы"], "budget", 2)
    )
...
async def q6(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "🎨 *Вопрос 7/20: Какие цвета хотите видеть в интерьере?\n\nНапишите через запятую:")
...
async def q7(msg: Message, state: FSMContext):
    ...
    await send_question(msg, "🚫 *Вопрос 8/20: Какие цвета НЕ хотите использовать?\n\nНапишите «нет» если таких нет")
...
async def q8(msg: Message, state: FSMContext):
    ...
    await send_question(msg, "☯️ *Вопрос 9/20: Вы предпочитаете светлые или тёмные тона?*")
    await msg.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Светлые тона", "Тёмные тона", "Смешанные"], "lightdark", 2)
    )
...
async def q9(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "📌 *Вопрос 10/20: Какие зоны должны быть в комнате?\n\nМожно выбрать несколько, потом нажмите «Готово»")
    await call.message.answer(
        "Выберите зоны:",
        reply_markup=inline_buttons(["Отдых", "Работа", "Приём гостей", "Хранение", "Обеденная", "Спорт"], "zone", 2)
    )
...
async def zones_done(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await send_question(call.message, "👥 *Вопрос 11/20: Сколько человек будут регулярно использовать комнату?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["1 человек", "2-3 человека", "4 и более"], "people", 2)
    )
...
async def q11(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "💡 *Вопрос 12/20: Какой тип освещения вам нравится?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Естественное + доп.", "Только верхний свет", "Много точечных", "Мягкий рассеянный", "Яркое белое", "Тёплое жёлтое"], "lighting", 2)
    )
...
async def q12(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "🪑 *Вопрос 13/20: Какая мебель обязательно должна быть в комнате?\n\nПеречислите через запятую (диван, кровать, шкаф, стол и т.д.)")
...
async def q13(msg: Message, state: FSMContext):
    ...
    await send_question(msg, "🔌 *Вопрос 14/20: Есть ли крупная техника, которую нужно вписать?\n\n(холодильник, стиральная машина, кондиционер, камин, проектор)\n\nЕсли нет — напишите «нет»")
...
async def q14(msg: Message, state: FSMContext):
    ...
    await send_question(msg, "🌿 *Вопрос 15/20: Важны ли экологичные или здоровые материалы?*")
    await msg.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Да, важны", "Нет, не принципиально"], "eco", 2)
    )
...
async def q15(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "🐾 *Вопрос 16/20: Есть ли домашние питомцы?*")
    await call.message.answer(
        "Выберите:",
        reply_markup=inline_buttons(["Кошка", "Собака", "Грызуны", "Нет"], "pets", 2)
    )
...
async def q16(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await send_question(call.message, "😞 *Вопрос 17/20: Что вам категорически не нравится в текущем интерьере?\n\nНапишите текстом (или «ничего»)")
...
async def q17(msg: Message, state: FSMContext):
    ...
    await send_question(msg, "❤️ *Вопрос 18/20: Что вы хотели бы сохранить из текущей обстановки?\n\n(например, любимое кресло, картину, ковёр)\n\nЕсли ничего — напишите «ничего»")
...
"""
    await send_question(msg, preview)
    await msg.answer(
        "Всё верно?",
        reply_markup=inline_buttons(["✅ Да, всё верно", "🔄 Начать заново"], "confirm", 2)
    )
...
async def get_photo(msg: Message, state: FSMContext):
    ...
    if len(photos) >= 3:
        await msg.answer("❌ Вы уже отправили 3 фото. Завершаем...")
    ...
    if len(photos) < 3:
        ...
        builder.adjust(1)
        await msg.answer(f"📸 Фото {len(photos)}/3 сохранено. Что дальше?", reply_markup=builder.as_markup())
    else:
...
async def add_more_photo(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("📸 Отправьте следующее фото:", reply_markup=nav_kb)
    await call.answer()
...
async def finish_and_send(msg: Message, state: FSMContext):
    ...
    if len(photos) == 0:
        await msg.answer("❌ Вы не отправили ни одного фото.")
        return
...
📌 *Чтобы отправить дизайн этому клиенту, используйте команду:* `/answer {msg.from_user.id}`
"""
...
async def wrong_photo(msg: Message):
    await msg.answer("❌ Пожалуйста, отправьте фото комнаты.")

# ========== АДМИН КОМАНДЫ ==========
@dp.message(Command("answer"))
async def cmd_answer(message: Message):
    """Отправить дизайн клиенту"""
    ...
    if len(args) != 2:
        await message.answer(
            "❌ *Использование:* `/answer USER_ID`\n\n"
            "Пример: `/answer 123456789`\n\n"
            "USER_ID можно найти в заявке клиента.",
            parse_mode="Markdown"
        )
        return
...
async def show_stats(message: Message):
    ...
    await message.answer(
        ...
        f"👥 Всего заявок: {len(user_forms)}\n"
        f"👑 Админов: {len(ADMIN_IDS)}",
        parse_mode="Markdown"
    )

# Обработка отправки дизайна (фото от админа)
@dp.message(F.photo)
async def handle_admin_photo(message: Message):
    """Если админ в режиме отправки дизайна, пересылаем фото клиенту"""
    if not is_admin(message.from_user.id):
        return
    
    # Проверяем, ждёт ли админ отправки дизайна
    ...
    try:
        ...
        await message.answer(f"❌ *Ошибка отправки:* {str(e)}\n\nПроверьте, что пользователь `{user_id}` существует и бот не заблокирован.", parse_mode="Markdown")
